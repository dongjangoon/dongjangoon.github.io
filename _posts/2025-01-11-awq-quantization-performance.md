...
[이전 글]({% post_url 2025-01-11-llm-inference-memory-bound %})에서 설명했듯이, LLM의 Decode 단계는 **Memory-Bound**입니다. 토큰 하나를 생성할 때마다 모델 전체 가중치를 메모리에서 읽어야 합니다.
...
```
RTX 5070 Ti 메모리 대역폭: ~896 GB/s

이론 최대 처리량:
  Qwen 3B FP16:   896 / 6 = 149 t/s
  Mistral 7B AWQ: 896 / 3.5 = 256 t/s  ← 1.7배!

실제 측정 (Batch=1):
  Qwen 3B FP16:   21 t/s (효율 14%)
  Mistral 7B AWQ: 31 t/s (효율 12%)
```
...
즉 AWQ(W4A16)에서 4비트는 **메모리 이득**만 가져오고 곱셈 자체는 FP16으로 돕니다. 저정밀을 하드웨어에서 직접 계산해 **연산 이득**까지 챙기려면 전용 텐서코어가 필요한데, 이 차이(native vs 에뮬레이션)와 NVFP4를 [4비트로 LLM 돌리기 (2) — GPU·SM·텐서코어, 그리고 native FP4]({% post_url 2026-06-26-nvfp4-2-gpu-tensor-core-hardware %})에서 자세히 다룹니다.
