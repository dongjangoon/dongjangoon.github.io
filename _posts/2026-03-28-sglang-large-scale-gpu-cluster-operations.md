...
EP에서 하나의 MoE 레이어를 처리하는 과정은 5단계를 거칩니다.
...
MoE 모델의 Router는 특정 전문가에 토큰이 편중되는 불균형을 만들어냅니다.
...
TBO는 피크 메모리를 절반으로 줄여 디바이스당 16,384 토큰을 처리할 수 있습니다 (비활성 시 8,192에서 OOM).
...
[이전 포스트]({{ site.baseurl }}{% post_url 2026-01-11-llm-inference-memory-bound %})에서 다뤘듯이, Prefill과 Decode는 연산 특성이 근본적으로 다릅니다.
...
Prefill 서버에서 계산된 KV cache를 Decode 서버로 전송해야 합니다. SGLang이 지원하는 전송 백엔드는 두 가지입니다.
...
두 백엔드 모두 **비동기 전송** 을 지원합니다. 별도 스레드 풀이 KV cache를 전송하므로, Prefill 서버는 다음 요청을 즉시 처리할 수 있습니다.
...
- **PD Disaggregation** 은 Prefill과 Decode를 물리적으로 분리하여 상호 간섭을 제거하고 독립적 스케일링을 실현합니다.
