...
> 3. 소프트웨어 편 — 커널이란 무엇인가, vLLM의 양자화 디스패치 체인
...
보통 SM은 4개의 **서브파티션(subpartition)** 으로 나뉘고, 각 서브파티션이 자체 워프 스케줄러와 텐서코어를 하나씩 가집니다 ([How To Scale Your Model — GPUs](https://jax-ml.github.io/scaling-book/gpus/)). 즉 GPU의 연산 능력은 결국 "SM이 몇 개냐 × SM 하나가 얼마나 일하느냐"로 정해집니다.

1편에서 말한 memory-bound 이야기를 떠올리면, 디코딩 단계에서는 이 수천 개의 코어가 한가하게 놀고 메모리 대역폭만 발목을 잡습니다 (이 부분은 [LLM 추론의 Memory-Bound 특성]({% post_url 2025-01-11-llm-inference-memory-bound %})에서 다뤘습니다). 그래서 양자화로 옮길 바이트를 줄이는 게 효과가 컸던 것이고, 이번 편의 텐서코어 이야기는 그 위에 **연산 자체도 빠르게** 만드는 층을 얹는 일입니다.
...
여기서 서빙 엔지니어가 정확히 구분해야 할 게 있습니다.

- **메모리 이득은 그대로 남습니다.** 어차피 메모리에서 옮길 때는 4비트라, 디코딩(memory-bound)에서는 여전히 빨라집니다. 7B AWQ 모델이 3B FP16보다 빠를 수 있는 이유가 바로 이것입니다 (→ [AWQ 양자화: 7B 모델이 3B보다 빠른 이유]({% post_url 2025-01-11-awq-quantization-performance %})).
...
정리하면, 같은 NVFP4 가중치라도 **옛 GPU에 올리면 메모리만 절약**되고, **Blackwell에 올려야 연산까지 빨라집니다.**
