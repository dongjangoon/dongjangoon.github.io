---
layout: single
title: "RadixAttention과 PagedAttention, KV 캐시 전략은 뭐가 다를까 (SGLang 시리즈 Part 2)"
...
이 글에서는 RadixAttention의 내부를 공부하며 정리한 내용을 풀어 보겠습니다. 비교 대상은 vLLM의 PagedAttention과 그 위에 구현된 APC(Automatic Prefix Caching)입니다.

두 시스템은 같은 문제를 다룹니다. **"LLM 서빙에서 KV cache를 어떻게 효율적으로 관리할 것인가?"** 다만 선택한 자료구조와 설계 방향이 꽤 다른 편이고, 제가 이해한 바로는 이 차이가 스케줄링, 캐시 재활용, 메모리 관리 전반에 영향을 주는 듯합니다.

## KV Cache는 왜 관리가 어려운가

[이전 포스트]({{ site.baseurl }}{% post_url 2025-11-04-kv-caching-gpu-parallelism %})에서 KV cache의 기본 구조와 메모리 계산을 다뤘습니다. 핵심만 짚으면 이렇습니다.
...
## RadixAttention의 가변 노드와 Radix Tree
...
두 접근법의 차이를 자료구조 관점에서 정리해 보겠습니다.
...
RadixAttention에서 흥미로웠던 부분은 단순한 KV cache 저장을 넘어, **스케줄러가 캐시 상태를 인지하고 요청 순서를 최적화**하는 점이었습니다.
...
이 전략은 **radix tree의 구조적 정보를 활용**하기 때문에 가능합니다. PagedAttention의 flat한 hash table에서는 요청 간의 prefix 관계를 직접 파악하기 어려워, 이런 최적화를 적용하기는 더 까다로운 편입니다.
...
아래 수치는 RunPod 등에서 공개한 벤치마크를 인용한 것으로, 환경에 따라 결과는 달라질 수 있습니다.
...
공개된 수치를 보면 캐시 미사용 시 두 엔진의 성능 차이는 미미하고, **차이는 주로 캐시 재활용 시나리오에서 나타나는** 것으로 보입니다.
...
- **Few-shot Learning 파이프라인**. 동일한 예시 세트를 공유하는 수백 건의 요청에서 큰 폭의 처리량 향상을 기대할 수 있습니다(논문 벤치마크에서는 4x 이상도 보고됩니다).
- **RAG 서비스**. 공통 system prompt + 검색 문서 prefix를 자동으로 캐싱합니다.
- **Agent / Tool Use**. ReAct 패턴에서 이전 사고 과정의 KV cache를 재활용하는데, 논문 벤치마크에서는 5x가량의 향상도 보고됩니다.
...
이 글에서 공부하며 정리한 핵심 차이를 짚어 보겠습니다.

**PagedAttention + APC**는 OS의 페이징 시스템처럼 **"블록 단위의 효율적 메모리 관리"** 에 무게를 둡니다. Hash table의 O(1) 룩업으로 빠르게 캐시를 찾지만, 블록 경계에서의 공유 제약과 flat 구조 때문에 요청 간 관계를 활용하기는 상대적으로 어려운 편입니다.

**RadixAttention**은 **"요청 간 prefix 관계의 구조적 표현과 활용"** 에 무게를 두는 쪽으로 이해했습니다. Tree 구조가 prefix 공유 관계를 명시적으로 드러내는 덕분에, 스케줄링(DFS_WEIGHT), 라우팅(Cache-Aware Load Balancer), eviction까지 캐시 상태를 인지한 최적화를 적용할 수 있습니다.

다음 **Part 3**에서는 SGLang의 또 다른 특징인 **Compressed Finite State Machine**을 정리해 보려 합니다. LLM에서 JSON 스키마 같은 구조화된 출력을 어떻게 효율적으로 보장하는지, 그리고 이것이 RadixAttention과 어떻게 결합되는지 공부하며 살펴볼 예정입니다.
