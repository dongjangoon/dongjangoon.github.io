...
이 전략은 **radix tree의 구조적 정보를 활용**하기 때문에 가능합니다. PagedAttention의 flat한 hash table에서는 요청 간의 prefix 관계를 직접 파악하기 어려워, 이런 최적화를 적용하기는 상대적으로 까다로운 편으로 보입니다.
...
이 글에서 공부하며 정리한 핵심 차이를 짚어 보면 다음과 같습니다.
...
**RadixAttention**은 **"요청 간 prefix 관계의 구조적 표현과 활용"** 에 무게를 두는 쪽으로 이해했습니다. Tree 구조가 prefix 공유 관계를 명시적으로 드러내어, 스케줄링(DFS_WEIGHT), 라우팅(Cache-Aware Load Balancer), eviction까지 캐시 상태를 인지한 최적화를 적용할 여지를 넓혀 줍니다.
