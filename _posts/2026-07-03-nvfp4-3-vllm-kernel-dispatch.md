...
[2편]({% post_url 2026-06-26-nvfp4-2-gpu-tensor-core-hardware %})은 한 가지 질문으로 끝났습니다. **하드웨어가 native FP4를 계산할 수 있어도, 그걸 실제로 호출하는 "커널"이 없으면 어떻게 될까요?** 그리고 vLLM은 내 GPU와 모델 포맷을 보고 어떤 커널을 고를까요?
...
> 2. [하드웨어 편]({% post_url 2026-06-26-nvfp4-2-gpu-tensor-core-hardware %}) — GPU / SM / 텐서코어, native FP4 vs 에뮬레이션
> 3. **소프트웨어 편 (이 글)** — 커널이란 무엇인가, vLLM의 양자화 디스패치 체인
> 4. 실측 편 — RTX 5070 Ti에서 NVFP4 직접 서빙하고 벤치마크
...
선택이 끝나면 forward(순전파)에서 레이어마다 그 커널이 호출됩니다. 그리고 이 GEMM 커널들 **위에** vLLM의 서빙 엔진이 얹힙니다. KV 캐시를 페이지 단위로 관리하는 **PagedAttention**(→ [vLLM PagedAttention]({% post_url 2025-11-04-vllm-pagedattention %}))과, 요청을 동적으로 묶는 **continuous batching**(→ [LLM 추론의 Memory-Bound 특성]({% post_url 2025-01-11-llm-inference-memory-bound %}), [vLLM 비동기 스케줄링]({% post_url 2026-02-08-vllm-async-scheduling %}))이 그 층입니다. 양자화 커널은 이 스택의 가장 아래에서 행렬곱을 담당하는 부품인 셈입니다.
...
이 구분은 [AWQ 글]({% post_url 2025-01-11-awq-quantization-performance %})에서 다룬 W4A16과 정확히 같은 이야기입니다. 그래서 "NVFP4 모델을 올렸는데 왜 기대만큼 안 빠르지?"의 답이 종종 여기에 있습니다. **하드웨어는 native FP4가 되는데, 디스패치가 Marlin으로 빠진 경우**입니다.

## 5. dense vs MoE, 그리고 SM120의 현실

같은 NVFP4라도 레이어 종류에 따라 커널 경로가 다릅니다.

- **dense 레이어** — 일반적인 행렬곱. `nvfp4_scaled_mm` 같은 scaled GEMM 커널을 씁니다.
- **MoE(Mixture-of-Experts) 레이어** — 토큰마다 일부 전문가(expert)만 골라 계산하는 구조라, 여러 전문가의 행렬곱을 묶어 처리하는 **grouped/fused MoE GEMM** 커널이 따로 필요합니다.
