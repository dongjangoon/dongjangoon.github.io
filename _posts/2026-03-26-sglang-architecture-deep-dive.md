---
layout: single
title: "SGLang 아키텍처 뜯어보기 - 논문과 내부 구조를 공부한 정리 (SGLang 시리즈 Part 1)"
...
SGLang은 이 문제를 정면에 두고 접근합니다. NeurIPS 2024에서 발표된 논문 *"SGLang: Efficient Execution of Structured Language Model Programs"* 는 **LLM 프로그램의 실행 효율성**이라는 관점에서 서빙 엔진을 재설계했습니다.

이 글에서는 논문의 핵심 아이디어부터 실제 구현의 아키텍처까지, 제가 공부하며 따라가 본 만큼 하나씩 정리해 봤습니다.

## 논문이 정의하는 LLM 프로그램 문제
...
│   ├── gen(), select(), fork(), join() Primitive
...
이 글에서는 Backend Runtime의 아키텍처를 중심으로 정리해 봅니다. Compressed FSM은 시리즈 Part 3에서 별도로 다룹니다.
...
SGLang은 `@sgl.function` 데코레이터로 LLM 프로그램을 정의합니다. 핵심 Primitive는 다섯 가지입니다.
...
각 Primitive의 역할을 정리하면 다음과 같습니다.

| Primitive | 역할 | Backend 연동 |
|-----------|------|-------------|
...
RadixAttention의 상세 구조와 PagedAttention과의 비교는 **Part 2** 에서 더 자세히 다뤄볼 예정입니다.
...
이 전략은 Decode 요청의 지연 시간(TPOT)을 안정적으로 유지하면서, Prefill에서는 cache 재활용을 극대화합니다.
...
핵심 아이디어는 **CPU가 항상 한 배치 앞서서 준비**한다는 데 있습니다.
...
아키텍처 정리를 마무리하며, 두 엔진의 설계 철학을 제가 이해한 만큼 비교해 보겠습니다.
...
이 글에서는 SGLang의 논문에서 제기한 문제의식, Frontend DSL의 설계, Backend Runtime의 전체 아키텍처, Overlap Scheduling, FlashInfer 통합, 그리고 vLLM과의 설계 철학 차이를 공부하며 정리해 봤습니다.

제가 이해한 핵심을 정리하면 다음과 같습니다.
...
다음 Part 2에서는 이 글에서 개념만 소개한 **RadixAttention**의 내부 구조를 좀 더 따라가 볼 생각입니다. Radix tree의 자료구조, prefix matching 알고리즘, cache-aware scheduling, 그리고 PagedAttention과의 벤치마크 비교를 다룰 예정입니다.
