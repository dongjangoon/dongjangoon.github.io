...
**초기 설정의 실수와 교훈**:
처음에는 파티션을 3개로 설정했다가, 로그 볼륨이 증가하면서 처리 지연이 발생했습니다. 이때 깨달은 것은:
...
Kafka를 활용한 로그 수집 아키텍처에서 가장 중요한 것은 **적절한 토픽 설계와 지속적인 모니터링**입니다. 특히 다음과 같은 점들이 핵심입니다:

### 설계 단계에서의 고려사항
1. **파티션 수 결정**: 현재 로드의 2배로 설정하여 확장성 확보
2. **보존 정책**: 비즈니스 요구사항과 스토리지 비용의 균형점 찾기
3. **압축 설정**: snappy 압축으로 성능과 저장 공간의 최적 균형

### 운영 단계에서의 핵심
1. **Consumer Lag 모니터링**: 시스템 건강성의 핵심 지표
2. **점진적 최적화**: 실제 사용 패턴을 바탕으로 한 설정 조정
3. **장애 대응**: 예상 시나리오별 대응 절차 사전 준비

실무에서 얻은 가장 큰 교훈은 **"완벽한 초기 설계보다는 적응적 운영이 더 중요하다"**는 것입니다. 로그 패턴과 볼륨은 서비스 성장과 함께 변화하므로, 모니터링 데이터를 바탕으로 한 지속적인 최적화가 필수입니다.

다음 포스트에서는 **"Kafka Connect를 활용한 OpenSearch 연동 최적화"**에 대해 다루겠습니다.
