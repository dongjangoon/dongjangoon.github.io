...
단적인 예로 Llama 3.1 70B 모델(BF16)을 H100에서 실행하는 경우를 살펴보겠습니다. 가중치만 140GB라 80GB짜리 H100 한 장에는 올라가지 않으므로, 두 장에 tensor parallel(TP=2)로 나눠 싣는다고 가정합니다.
```
모델 크기: 70B × 2 bytes (BF16) = 140GB
GPU 메모리: 80GB × 2장 = 160GB
메모리 대역폭: 3.35 TB/s × 2장 = 6.7 TB/s

토큰 1개 생성 시:
- 메모리 읽기 시간: 140GB / 6,700GB/s ≈ 21ms
- 실제 연산 시간: 140B FLOPs / (2 × 989 TFLOPS) ≈ 0.07ms
...
단일 요청 이론 최대: 896 / 6 = 149 tokens/s
실제 측정 (Batch=1): 21 tokens/s (14% 활용)
...
```
RTX 5070 Ti (896 GB/s)  → ~27 t/s
A100 (2,039 GB/s)       → ~60 t/s
H100 (3,350 GB/s)       → ~100 t/s
```
