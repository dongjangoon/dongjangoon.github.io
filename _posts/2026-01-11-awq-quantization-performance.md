...
[이전 글]({% post_url 2026-01-11-llm-inference-memory-bound %})에서 설명했듯이, LLM의 Decode 단계는 **Memory-Bound**입니다. 토큰 하나를 생성할 때마다 모델 전체 가중치를 메모리에서 읽어야 합니다.
...
즉 AWQ(W4A16)에서 4비트는 **메모리 이득**만 가져오고 곱셈 자체는 FP16으로 돕니다. 저정밀을 하드웨어에서 직접 계산해 **연산 이득**까지 챙기려면 전용 텐서코어가 필요한데, 이 차이(native vs 에뮬레이션)와 NVFP4를 [4비트로 LLM 돌리기 (2) — GPU·SM·텐서코어, 그리고 native FP4]({% post_url 2026-06-19-nvfp4-2-gpu-tensor-core-hardware %})에서 자세히 다룹니다.
