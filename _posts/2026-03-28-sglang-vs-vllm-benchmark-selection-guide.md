---
layout: single
title: "SGLang vs vLLM, 언제 뭘 쓸까 - 벤치마크 보며 정리한 메모 (SGLang 시리즈 Part 5)"
...
[Part 1]({{ site.baseurl }}{% post_url 2026-03-26-sglang-architecture-deep-dive %})부터 [Part 4]({{ site.baseurl }}{% post_url 2026-03-28-sglang-large-scale-gpu-cluster-operations %})까지 SGLang의 아키텍처, RadixAttention, Compressed FSM, 대규모 GPU 운영을 살펴봤습니다. 이번 편에서는 자연스럽게 따라오는 질문을 제가 본 만큼 정리해보려 합니다.

**"우리 서비스에는 SGLang과 vLLM 중 무엇이 적합한가?"**

이 질문에 "항상 X가 좋다"라는 답은 없는 것 같습니다. 모델, 하드웨어, 워크로드 유형, 동시 접속 수에 따라 답이 달라지기 때문입니다. 이 글에서는 여러 곳에 공개된 벤치마크 데이터와 제가 직접 돌려본 작은 테스트를 모아, 시리즈에서 살펴본 아키텍처 차이가 실제 성능에 어떻게 반영되는지를 제 나름대로 연결해 정리합니다. 종합적인 결론이라기보다 공부하며 남기는 메모에 가깝습니다.
...
여러 출처에 공개된 벤치마크를 모델과 GPU별로 모아봤습니다. 아래 수치는 모두 표의 출처에서 보고한 값이며, 측정 환경과 버전에 따라 달라질 수 있습니다.
...
이 표만 보면 소형 모델에서 SGLang의 이점이 뚜렷해 보입니다. 특히 동시 요청 처리에서 4.6배 차이가 나는 것(DEV Community 보고)은 Overlap Scheduling과 효율적인 배치 구성의 효과로 볼 수 있습니다.
...
**p50(중앙값)** 에서는 두 엔진이 비슷하지만, 출처에 따라 **p99(꼬리 레이턴시)에서 vLLM이 80% 더 높게** 나온 경우도 있습니다(ersteiger). 이런 차이가 재현된다면 SLA가 중요한 프로덕션 환경에서는 의미가 있을 수 있습니다.
...
이 표들에서는 SGLang의 per-token 레이턴시가 전반적으로 더 안정적으로 보입니다. **4~21ms 범위** 내에서 비교적 일관된 성능을 보이는 반면, vLLM은 변동폭이 더 큰 편입니다. Overlap Scheduling 덕분인 듯합니다.
...
PremAI가 보고한 수치에서는 RadixAttention의 tree 기반 자동 prefix 발견과 cache-aware scheduling(DFS_WEIGHT)이 캐시 히트율에서 **3~6배 차이** 로 나타났습니다. 이 정도 차이는 워크로드 구성에 크게 의존하므로 환경에 따라 달라질 수 있습니다.
...
LMSYS 벤치마크의 few-shot 시나리오에서 나타난 3~4.4배의 처리량 차이는 Part 2에서 살펴본 RadixAttention의 구조적 이점과 잘 들어맞습니다. 동일한 few-shot 예시의 KV cache를 tree 구조로 자동 공유하고, DFS_WEIGHT 스케줄링으로 캐시 히트를 끌어올린 결과인 듯합니다.
...
| **주요 메트릭** | `num_requests_running`, `kv_cache_usage_perc`, latency histograms | phase labels (prefill/decode), Router 40+ 메트릭 |
| **Grafana 대시보드** | 공식 제공 | 커뮤니티 |

두 엔진 모두 Prometheus + Grafana 통합을 지원합니다. vLLM이 기본 활성이라 초기 설정이 간단하고, SGLang은 Router 레벨에서 더 세밀한 메트릭을 제공합니다.
...
앞에서 모아본 공개 벤치마크는 전부 H100, A100 같은 데이터센터 GPU 기준입니다. 그래서 호기심에 직접 돌려봤습니다. RTX 5070 Ti 한 장에 Mistral-7B-AWQ를 올려서, 앞서 살펴본 아키텍처 차이가 실제 숫자로 어떻게 나타나는지 확인해보려고 합니다. 미리 말씀드리면 이건 엄밀한 벤치마크가 아니라 GPU 한 장으로 돌려본 작은 개인 테스트입니다.
...
가장 큰 건 **vLLM v0.18의 V1 스케줄러** 의 영향입니다. vLLM은 v0.17에서 V1 아키텍처로 전환하면서 스케줄러를 재설계했습니다. 공개 벤치마크 중 상당수는 이전 버전 기준이라, v0.18에서는 결과가 달라질 수 있습니다.

여기에 **AWQ 커널 성숙도 차이** 가 더해집니다. SGLang 서버 시작 시 로그에 `"awq quantization is not fully optimized yet"` 경고가 찍혔습니다. SGLang은 `awq_marlin` 백엔드를 권장하지만, 이번 테스트에서는 `awq`를 명시적으로 지정했기 때문에 최적화되지 않은 경로를 탔습니다. vLLM의 AWQ 커널이 더 성숙한 상태입니다.

**KV cache 슬롯 부족** 도 영향을 줬을 것입니다. SGLang의 `max_total_tokens=8192` 설정에서 동시 요청이 많아지면 KV cache가 부족해져 요청이 큐에 쌓이게 됩니다. 동시성 16에서 TTFT가 687ms로 급등한 건 이 때문으로 보입니다.
...
그리고 RTX 5070 Ti의 메모리 대역폭(896 GB/s)은 H100(3.35 TB/s)의 27% 수준입니다. 메모리 바운드 특성이 강한 LLM 추론에서 이 차이가 스케줄러 최적화의 효과를 상쇄하거나 증폭시킬 수 있다는 점도 감안해야 합니다. 소비자 GPU에서의 결과를 데이터센터 환경에 그대로 대입하기는 어렵습니다.
...
두 엔진 중 하나를 선택해야 한다면, 제가 본 만큼은 **워크로드의 특성에서 답을 찾는 게 가장 합리적**이라고 봅니다. prefix 공유가 빈번하거나, 구조화 출력이 핵심이거나, 대형 MoE 모델을 서빙해야 한다면 SGLang이 아키텍처적으로 유리한 편입니다. 극단적 동시성, AMD GPU, 또는 성숙한 K8s 생태계가 필요하다면 vLLM이 더 무난한 선택일 수 있습니다. 다만 제 작은 테스트에서도 드러났듯, 같은 모델이라도 양자화 백엔드와 엔진 버전에 따라 결과가 크게 달라지므로 실제 워크로드로 직접 확인해보는 게 가장 확실합니다.

무엇을 선택하든, 이 시리즈에서 살펴본 아키텍처적 차이를 이해하고 있다면 **왜 그 엔진이 특정 시나리오에서 더 나은지** 를 스스로 설명해볼 수 있을 거라 생각합니다. 저도 그걸 목표로 이 시리즈를 정리했습니다.
