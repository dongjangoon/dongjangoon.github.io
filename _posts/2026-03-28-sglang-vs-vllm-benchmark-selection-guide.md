...
이 표만 보면 소형 모델에서 SGLang의 이점이 뚜렷해 보입니다. 특히 동시 요청 처리에서 4.6배 차이가 나는 것(DEV Community 보고)은 Overlap Scheduling과 효율적인 배치 구성의 효과로 해석할 수 있습니다.
...
이 표들에서는 SGLang의 per-token 레이턴시가 전반적으로 더 안정적으로 보입니다. **4~21ms 범위** 내에서 비교적 일관된 성능을 보이는 반면, vLLM은 변동폭이 더 큰 편입니다. Overlap Scheduling의 효과로 해석할 수 있는 부분입니다.
...
LMSYS 벤치마크의 few-shot 시나리오에서 나타난 3~4.4배의 처리량 차이는 Part 2에서 살펴본 RadixAttention의 구조적 이점과 잘 들어맞습니다. 동일한 few-shot 예시의 KV cache를 tree 구조로 자동 공유하고, DFS_WEIGHT 스케줄링으로 캐시 히트를 끌어올린 결과로 보입니다.
...
두 엔진 중 하나를 선택해야 한다면, 제가 본 만큼은 **워크로드의 특성에서 답을 찾는 게 가장 합리적**으로 보입니다. prefix 공유가 빈번하거나, 구조화 출력이 핵심이거나, 대형 MoE 모델을 서빙해야 한다면 SGLang이 아키텍처적으로 유리한 편입니다. 극단적 동시성, AMD GPU, 또는 성숙한 K8s 생태계가 필요하다면 vLLM이 더 무난한 선택일 수 있습니다. 다만 제 작은 테스트에서도 드러났듯, 같은 모델이라도 양자화 백엔드와 엔진 버전에 따라 결과가 크게 달라지므로 실제 워크로드로 직접 확인해보는 게 가장 확실합니다.
